...
namespace CWA.Venda.Data
{
    public class CuboReciboJCData
    {
        ...
        public List<CuboReciboJCEntity> GetCuboReciboJCEntityList(string psDtInicioInic = "", string psDtInicioFim = "", Int32 psCdcontabil = 0)
        {
            try
            {
                _erro = 0;
                ...
                if (DataContext.AbrirConexao())
                {
                    ...
                    myComando.CommandText = "WS_CUBO_RECIBO_JC";
                    ...
                    if (DataContext.Erro == 0)
                    {
                        ...
                        if (myReader.HasRows)
                        {
                            ...
                            CuboReciboJCEntity lEnt;

                            while (myReader.Read())
                            {

                                lEnt = new CuboReciboJCEntity();

                                lEnt.COD_CLIENTE = int.Parse(myReader[0].ToString());
                                lEnt.CONTRATO = myReader[1].ToString();
                                lEnt.NU_RECIBO = myReader[2].ToString();
                                lEnt.NU_PARCELA = myReader[3].ToString();
                                lEnt.VA_PARCELA = myReader[4].ToString();
                                lEnt.DT_VENCIMENTO = myReader[5].ToString();
                                lEnt.DT_BAIXA = myReader[6].ToString();
                                lEnt.ST_PARCELA = myReader[7].ToString();
            ...
            {
                _erro = -99;
                _msgErro = "CuboReciboJCData (Lista) - " + ex.Message;
            ...
            finally
            {
                if (DataContext.ConexaoAberta())
                {
                    ...
                    {
                        _erro = -1;
                        _msgErro = "CuboReciboJCData (Lista) - Erro ao fechar conexão com o banco de dados.";
                    }
                }
            }
        }
